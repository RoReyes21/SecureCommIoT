...
// CAMBIO IMPORTANTE: Ahora devuelve bool y pasa el mensaje descifrado por referencia
bool decrypt_message(const std::vector<unsigned char>& rx_key, 
                     const std::vector<unsigned char>& ciphertext, 
                     const std::vector<unsigned char>& nonce, 
                     std::string& decrypted_message_out); // Este parámetro contendrá el mensaje descifrado si la autenticación es exitosa
